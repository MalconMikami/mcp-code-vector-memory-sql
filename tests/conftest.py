...
def server_module(tmp_path, monkeypatch):
    monkeypatch.setenv("CODE_MEMORY_DB_URL", f"file:{tmp_path / 'test.db'}")
    monkeypatch.setenv("CODE_MEMORY_DB_AUTH_TOKEN", "")
    ...
    monkeypatch.setenv("CODE_MEMORY_NER_MODEL", "")
    ...
    server.store = server.MemoryStore(embedder=server.EmbeddingModel(embed_dim=server.EMBED_DIM))
    return server
